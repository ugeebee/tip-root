...
export default function SettingsPage() {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            try {
                // Fetch Profile Data
                const settingsRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/dashboard/settings`, {
                    credentials: 'include'
                });
                if (settingsRes.ok) {
                ...
                // Fetch Secure Token
                const tokenRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/dashboard/token`, {
                    credentials: 'include'
                });
                if (tokenRes.ok) {
    ...
    const handleRotateToken = async () => {
        ...
        setIsRotating(true);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/dashboard/token/rotate`, {
                method: 'POST',
                credentials: 'include'
            });
            if (res.ok) {
