...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // --- NEW: Silent Token Refresher ---
  useEffect(() => {
    // 1. Refresh immediately on mount (in case they left the tab open overnight)
    const refreshSession = async () => {
      try {
        await fetch('https://streamer.tip-root.in/api/auth/refresh', {
          method: 'POST',
          credentials: 'include' // This sends the 7-day refresh cookie!
        });
      } catch (err) {
    ...
    // 2. Set an interval to refresh every 10 minutes (safely before the 15m expiry)
